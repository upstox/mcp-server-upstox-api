...
import { getOrderHistoryHandler } from "../src/tools/get-order-history";
...
describe("getOrderHistoryHandler", () => {
  ...
  it("should require an access token", async () => {
    await expect(getOrderHistoryHandler({ accessToken: "", orderId: "123" }, {})).rejects.toThrow("Access token is required");
  });

  it("should require an order ID", async () => {
    await expect(getOrderHistoryHandler({ accessToken: "valid-token", orderId: "" }, {})).rejects.toThrow("At least one of 'orderId' or 'tag' is required");
  });

  it("should successfully fetch order history", async () => {
    const mockResponse = {
      status: "success",
      data: [
        {
    ...
    const result = await getOrderHistoryHandler({ accessToken: "valid-token", orderId: "220728000000001" }, {});
  ...
  it("should handle API errors", async () => {
    ...
    await expect(getOrderHistoryHandler({ accessToken: "invalid-token", orderId: "123" }, {})).rejects.toThrow("Error occurred while calling Upstox API");
  });
