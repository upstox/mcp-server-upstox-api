...
import { getOrderBookHandler } from "../src/tools/get-order-book";

describe("getOrderBookHandler", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });

  it("should require an access token", async () => {
    await expect(getOrderBookHandler({ accessToken: "" }, {})).rejects.toThrow("Access token is required");
  });
  ...
  it("should successfully fetch order book", async () => {
    ...
    const result = await getOrderBookHandler({ accessToken: "valid-token" }, {});
  ...
  it("should handle API errors", async () => {
    ...
    await expect(getOrderBookHandler({ accessToken: "invalid-token" }, {})).rejects.toThrow("Error occurred while calling Upstox API");
  });
