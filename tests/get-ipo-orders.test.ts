...
import { getIpoOrdersHandler } from "../src/tools/get-ipo-orders";
...
describe("getIpoOrdersHandler", () => {
  ...
  it("should return an API error when the KV namespace is unavailable", async () => {
    const result = await getIpoOrdersHandler({}, { props: { sessionId: "session-1" }, env: {} });
    expect(result.isError).toBe(true);
  ...
  it("should return an authentication error when the session has expired", async () => {
    const result = await getIpoOrdersHandler({}, buildExtra(null));
    expect(result.isError).toBe(true);
    expect(result.metadata?.errorType).toBe("AUTHENTICATION_EXPIRED");
    expect(result.metadata?.requiresReauth).toBe(true);
  });

  it("should successfully fetch IPO orders", async () => {
    mockFetchOk();
  ...
  it("should omit pagination parameters that were not supplied", async () => {
    mockFetchOk();

    await getIpoOrdersHandler({}, buildExtra());
  ...
  it("should pass the supplied pagination parameters through to the query string", async () => {
    ...
    expect(url.searchParams.get("records")).toBe("25");
  });
  ...
  it("should reject records above the documented maximum of 30", async () => {
    await expect(
      getIpoOrdersHandler({ records: 31 }, buildExtra())
    ).rejects.toThrow();
  });
  ...
  it("should validate the response data structure", async () => {
    ...
    if (content.type === "text") {
      ...
      expect(parsedData).toHaveProperty("meta_data");

      const application = parsedData.data[0];
      ...
      expect(application.bids[0]).toHaveProperty("price");
