...
import { getOrderTradesHandler } from "../src/tools/get-order-trades";

describe("getOrderTradesHandler", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });

  it("should require an access token", async () => {
    await expect(getOrderTradesHandler({ accessToken: "", orderId: "123" }, {})).rejects.toThrow("Access token is required");
  });

  it("should require an order ID", async () => {
    await expect(getOrderTradesHandler({ accessToken: "valid-token", orderId: "" }, {})).rejects.toThrow("Order ID is required");
  });

  it("should successfully fetch order trades", async () => {
    const mockResponse = {
      status: "success",
      data: [
        {
    ...
    const result = await getOrderTradesHandler({ accessToken: "valid-token", orderId: "220728000000001" }, {});
  ...
  it("should handle API errors", async () => {
    ...
    await expect(getOrderTradesHandler({ accessToken: "invalid-token", orderId: "123" }, {})).rejects.toThrow("Error occurred while calling Upstox API");
  });
